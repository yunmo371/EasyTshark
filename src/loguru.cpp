            ...
            (st.st_ino != file_abs->st.st_ino))
        {
            ...
            if (!file_abs->fp)
            {
                VLOG_F(g_internal_verbosity,
                       "Reopening file '" LOGURU_FMT(s) "' due to previous error", file_abs->path);
            }
            else if (ret < 0)
            {
            ...
            {
                VLOG_F(g_internal_verbosity,
                       "Reopening file '" LOGURU_FMT(s) "' due to file changed", file_abs->path);
            }
        ...
        for (int arg_it = 1; arg_it < argc; ++arg_it)
        {
            ...
            if (strncmp(cmd, verbosity_flag, arg_len) == 0 && !last_is_alpha)
            {
                ...
                if (value_str[0] == '\0')
                {
                    ...
                    arg_it += 1;
                    CHECK_LT_F(arg_it, argc, "Missing verbosiy level after " LOGURU_FMT(s) "",
                               verbosity_flag);
...
#define getcwd _getcwd
    ...
    bool add_file(const char* path_in, FileMode mode, Verbosity verbosity)
    {
        ...
        VLOG_F(g_internal_verbosity,
               "Logging to '" LOGURU_FMT(s) "', mode: '" LOGURU_FMT(s) "', verbosity: " LOGURU_FMT(
                   d) "",
    ...
    */
    ...
    bool add_syslog(const char* app_name, Verbosity verbosity, int facility)
    {
        ...
        VLOG_F(g_internal_verbosity, "Logging to 'syslog' , verbosity: " LOGURU_FMT(d) "",
               verbosity);
        return true;
#else
        (void)app_name;
...
#if LOGURU_USE_FMTLIB
            auto buff =
                textprintf("{:.{}f} s: {:s}", duration_sec, LOGURU_SCOPE_TIME_PRECISION, _name);
#else
...
#if LOGURU_USE_FMTLIB
                auto prefix =
                    textprintf("[ErrorContext] {.{}s}:{:-5u} {:-20s} ", filename(entry->_file),
                               LOGURU_FILENAME_WIDTH, entry->_line, description.c_str());
#else
                auto prefix = textprintf("[ErrorContext] %*s:%-5u %-20s ", LOGURU_FILENAME_WIDTH,
