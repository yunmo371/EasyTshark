    ...
    public:
            ...
            , nullable_(false)
        {
            ...
            if (const ValueType* v = GetMember(value, GetPatternString()))
                pattern_ =
                    CreatePattern(*v, schemaDocument, p.Append(GetPatternString(), allocator_));
...
#undef RAPIDJSON_STRING_
    ...
    private:
        ...
        // required. Also creates a hasher for enums and array uniqueness, if required. Also a
        // useful place to add type-independent error checks.
...
public:
    ...
    */
    ...
    void SchemaErrorValue(const SchemaErrorCode code, const PointerType& location, const Ch* value,
                          SizeType length)
    {
        currentError_ = GValue(kObjectType);
        currentError_.AddMember(GetValueString(), GValue(value, length, *allocator_).Move(),
                                *allocator_);
        AddCurrentError(code, location);
    }

    //! Method for error with invalid pointer
    void SchemaErrorPointer(const SchemaErrorCode code, const PointerType& location,
                            const Ch* value, SizeType length, const PointerType& pointer)
    {
        currentError_ = GValue(kObjectType);
        currentError_.AddMember(GetValueString(), GValue(value, length, *allocator_).Move(),
                                *allocator_);
...
private:
    ...
    void AddErrorInstanceLocation(GValue& result, const PointerType& location)
    {
        ...
        GValue instanceRef(sb.GetString(), static_cast<SizeType>(sb.GetSize() / sizeof(Ch)),
                           *allocator_);
...
#define RAPIDJSON_STRING_(name, ...)                                                               \
    ...
    RAPIDJSON_STRING_(SpecUnsupported, 'S', 'p', 'e', 'c', 'U', 'n', 's', 'u', 'p', 'p', 'o', 'r',
                      't', 'e', 'd')
    ...
    RAPIDJSON_STRING_(RefPointerInvalid, 'R', 'e', 'f', 'P', 'o', 'i', 'n', 't', 'e', 'r', 'I', 'n',
                      'v', 'a', 'l', 'i', 'd')
    RAPIDJSON_STRING_(RefUnknown, 'R', 'e', 'f', 'U', 'n', 'k', 'n', 'o', 'w', 'n')
    RAPIDJSON_STRING_(RefCyclical, 'R', 'e', 'f', 'C', 'y', 'c', 'l', 'i', 'c', 'a', 'l')
    RAPIDJSON_STRING_(RefNoRemoteProvider, 'R', 'e', 'f', 'N', 'o', 'R', 'e', 'm', 'o', 't', 'e',
                      'P', 'r', 'o', 'v', 'i', 'd', 'e', 'r')
    RAPIDJSON_STRING_(RefNoRemoteSchema, 'R', 'e', 'f', 'N', 'o', 'R', 'e', 'm', 'o', 't', 'e', 'S',
                      'c', 'h', 'e', 'm', 'a')
    RAPIDJSON_STRING_(ReadOnlyAndWriteOnly, 'R', 'e', 'a', 'd', 'O', 'n', 'l', 'y', 'A', 'n', 'd',
                      'W', 'r', 'i', 't', 'e', 'O', 'n', 'l', 'y')
...
#undef RAPIDJSON_STRING_
                                ...
                                const ValueType& v, const ValueType& document, const UriType& id)
    {
        ...
        GenericStringBuffer<EncodingType> sb;
        pointer.StringifyUriFragment(sb);
        RAPIDJSON_SCHEMA_PRINT(Method, "GenericSchemaDocument::CreateSchema", sb.GetString(),
                               id.GetString());
        if (v.IsObject())
        {
                         ...
                         const ValueType& document, const UriType& id)
    {
        ...
        RAPIDJSON_SCHEMA_PRINT(Method, "GenericSchemaDocument::HandleRefSchema", sb.GetString(),
                               id.GetString());
        ...
        if (itr->value.IsString())
        {
            ...
            else
            {
                ...
                RAPIDJSON_SCHEMA_PRINT(SchemaIds, id.GetString(), itr->value.GetString(),
                                       ref.GetString());
                    ...
                    else
                    {
                        ...
                        // document and its absolute JSON pointer.
                                     ...
                                     FindId(*base, ref, pointer,
                                            UriType(ref.GetBaseString(), ref.GetBaseStringLength(),
                                                    allocator_),
                                            true, basePointer))
                        {
                            if (IsCyclicRef(pointer))
    ...
    ValueType* FindId(const ValueType& doc, const UriType& finduri, PointerType& resptr,
                      const UriType& baseuri, bool full,
                      const PointerType& here = PointerType()) const
    {
        ...
        if (doc.GetType() == kObjectType)
        {
            ...
            for (m = doc.MemberBegin(); m != doc.MemberEnd(); ++m)
            {
                ...
                {
                    resval = FindId(
                        m->value, finduri, resptr, localuri, full,
...
private:
    ...
    void AddErrorArray(const ValidateErrorCode code, ISchemaValidator** subvalidators,
                       SizeType count)
    {
        ValueType errors(kArrayType);
        for (SizeType i = 0; i < count; ++i)
